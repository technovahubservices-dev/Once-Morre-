...
export default function ProductCard({ product, variant = 'default' }) {
  ...
  return (
    <>
      <div className="group bg-surface-white border border-surface-container-highest overflow-hidden luxury-hover transition-all duration-300">
        <div className="relative overflow-hidden">
          ...
          <button
            onClick={() => toggleWishlist(product)}
            className="absolute top-4 right-4 bg-surface-white/80 backdrop-blur-sm p-2 rounded-full hover:bg-surface-white transition-colors"
          >
          ...
          <button
            ...
            className="absolute bottom-4 left-4 right-4 bg-deep-emerald text-surface-white font-label-caps text-label-caps uppercase tracking-widest py-3 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          >
            Add to Cart
          </button>
          <button
            onClick={() => setShowQuickView(true)}
            className="absolute bottom-4 left-4 right-4 bg-surface-white text-deep-emerald font-label-caps text-label-caps uppercase tracking-widest py-3 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 border border-deep-emerald"
          >
            Quick View
          </button>
        </div>
        <div className="p-6">
          ...
          {/* // Fix price alignment: consistent ₹ currency, tabular-nums prevents digit jitter,
              // removed justify-between so price and link stack cleanly on narrow cards. */}
