    ...
    <div className="bg-surface-white border border-outline-variant p-8 shadow-sm sticky top-32">
      ...
      <div className="space-y-4 mb-6 font-body-md text-body-md">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-on-surface tabular-nums">₹ {cartTotal.toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-on-surface-variant">Making Charges</span>
          <span className="text-on-surface tabular-nums">₹ {makingCharges.toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-on-surface-variant">Tax</span>
          <span className="text-on-surface tabular-nums">₹ {tax.toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-on-surface-variant">Delivery</span>
          <span className="text-primary font-semibold">{delivery === 0 ? 'Free' : `₹ ${delivery}`}</span>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <span className="font-headline-md text-headline-md text-deep-emerald tabular-nums">₹ {total.toLocaleString()}</span>
      </div>
      <Link
        to="/checkout"
